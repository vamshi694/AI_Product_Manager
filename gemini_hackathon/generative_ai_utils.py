...
def generate_user_stories(chat_conv_history, generated_epic):
    ...
    user_stories = model.generate_content(f"Create 2 short user stories for the requested feature based on the conversation and the epic created: {chat_conv_history} + {generated_epic}. Respond in JSON format only. List of dictionaries with each dictionary consisting of 'user_story_name' as key and 'user_story_description' as value. No subheadings. No additional text or explanations are required.")
